...
which goes this way: given a positive number stem:[n],
if it is odd, mutliply it by 3 and add 1
(hence the name _stem:[3n + 1] problem_) and repeat,
if it is even, divide by 2 and repeat.
The Collatz conjecture states that for every starting stem:[n],
you will eventually reach the cycle stem:[1, 2, 4].
Let's implement a program which, for given stem:[n],
computes the number of steps until the sequence reaches 1.
(Example: for stem:[n = 6], the sequence is stem:[3, 10, 5, 16, 8, 4, 2, 1]
...
The program which, for given stem:[n], computes the lenght of
stem:[3n + 1] sequence and prints its length, is
...
(you would need to find a very large stem:[n] to overflow the stack though).
...
[stem]
++++
\big[p \twoheadrightarrow \big(c \mapsto g(c)\big) \big] \twoheadrightarrow \big(b \mapsto f(b)\big) \equiv p \twoheadrightarrow \Big[ c \mapsto \Big(g(c) \twoheadrightarrow \big(b \mapsto f(b)\big)\Big) \Big]
++++

where stem:[\twoheadrightarrow] stands for `flatMap`